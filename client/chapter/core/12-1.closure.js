...
function first() {
  ...
  return second; //함수 실행 값이 아니라 함수를 리턴
}
//second 함수를 실행하려면 first()()를 해야 함(커링 함수). 이렇게 쓰지 않고 아래처럼 다른 변수에 할당해서 씀

const value = first(); //value는 함수가 됨(seconde()함수 전체를 받음)

// 전역의 오염을 막기 위해 클로저 사용

// let count = 0;
// function number() {
//   return ++count;
// }
//이 상태면 어디에서든 count에 접근할 수 있기 때문에 원하는 값을 도출하지 못할 수 있음
...
function number() {
  ...
  //함수 이름은 필요 없기 때문에 이렇게 쓸 수 있음
  // return function(){
  //   return ++count;
  // }

  //this가 없는 경우 화살표 함수로 써도 괜찮음
  return () => ++count;
}

const counter = number(); //inner() 함수 할당. counter는 함수의 능력을 쓸 수 있음
// console.log(count); //안 됨
// 계속 counter가 number()를 계속 참조하고 있기 때문에 가비지 컬렉터가 접근할 수 없음 => 누구도 들어갈(접근) 수 없게 만든 것과 같음 (closure)
